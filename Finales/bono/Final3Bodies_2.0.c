...
int main(int argc, char **argv){
  ...
  int k;
  for( k = 0; k < n_masses - 1; k++){

    fprintf(data1, "%le %le %le ",r[0][k],r[1][k],r[2][k]);
  
  }

  fprintf(data1, "%le %le %le %le\n",r[0][n_masses-1],r[1][n_masses-1],r[2][n_masses-1],actualTime);
...
void updateAcc( FLOAT **r, FLOAT **a , FLOAT *m ){
  ...
  for( i = 0; i < n_masses; i++ ){
    ...
    for( j = 0; j < n_masses; j++ ){
      if( i != j ){
	FLOAT normi = pow(norm((r[0][i] - r[0][j]),(r[1][i] - r[1][j]),(r[2][i] - r[2][j])) ,3)+ 0.01;
