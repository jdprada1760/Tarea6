...
int main( int argc, char **argv){
  ...
  FLOAT tmp1;
  int id;
  int test = fscanf(data, "%d %le %le %le %le %le %le %le\n", &id, &tmp1, &tmp1, &tmp1, &tmp1, &tmp1, &tmp1, &tmp1);
  if( id < 0 ){
  ...
  FLOAT *Temp = malloc( 7*sizeof(FLOAT));
  int index = 0;
  int Index = 0;
...
void updateAcc( FLOAT **rm, FLOAT **rM, FLOAT **am, FLOAT  **aM, FLOAT *M, int n_masses, int n_Masses ){
  ...
  for( i = 0; i < n_Masses; i++ ){
	...
	FLOAT normi = pow(norm((rM[0][i] - rM[0][j]),(rM[1][i] - rM[1][j]),(rM[2][i] - rM[2][j])),3);
...
void rungeKutta4( FLOAT **rm, FLOAT **vm, FLOAT **rM, FLOAT **vM, FLOAT *M, FLOAT dt, int n_masses, int n_Masses ){
  ...
  for( i = 0; i < n_Masses; i++ ){
    ...
    newRM[2][i] = rM[2][i] + krM[0][2][i]*dt/2 ;
    krM[1][0][i]  = vM[0][i] + kvM[0][0][i]*dt/2 ;
    krM[1][1][i]  = vM[1][i] + kvM[0][1][i]*dt/2 ;
    krM[1][2][i]  = vM[2][i] + kvM[0][2][i]*dt/2 ;
  ...
  for( i = 0; i < n_Masses; i++ ){
    ...
    newRM[2][i] = rM[2][i] + krM[1][2][i]*dt/2 ;
    krM[2][0][i]  = vM[0][i] + kvM[1][0][i]*dt/2 ;
    krM[2][1][i]  = vM[1][i] + kvM[1][1][i]*dt/2 ;
    krM[2][2][i]  = vM[2][i] + kvM[1][2][i]*dt/2 ;
  ...
  for( i = 0; i < n_Masses; i++ ){
    ...
    newRM[2][i] = rM[2][i] + krM[2][2][i]*dt ;
    krM[3][0][i]  = vM[0][i] + kvM[2][0][i]*dt ;
    krM[3][1][i]  = vM[1][i] + kvM[2][1][i]*dt ;
    krM[3][2][i]  = vM[2][i] + kvM[2][2][i]*dt ;
  ...
  for( i = 0; i < n_Masses; i++ ){
    ...
    FLOAT kmeanx = (1/6.0)*(krM[0][0][i] + 2*krM[1][0][i] + 2*krM[2][0][i] + krM[3][0][i]);
    rM[0][i] = rM[0][i] + kmeanx*dt;
    FLOAT kmeany = (1/6.0)*(krM[0][1][i] + 2*krM[1][1][i] + 2*krM[2][1][i] + krM[3][1][i]);
    rM[1][i] = rM[1][i] + kmeany*dt;
    FLOAT kmeanz = (1/6.0)*(krM[0][2][i] + 2*krM[1][2][i] + 2*krM[2][2][i] + krM[3][2][i]);
    rM[2][i] = rM[2][i] + kmeanz*dt;
    FLOAT kmeanvx = (1/6.0)*(kvM[0][0][i] + 2*kvM[1][0][i] + 2*kvM[2][0][i] + kvM[3][0][i]);
    vM[0][i] = vM[0][i] + kmeanvx*dt;
    FLOAT kmeanvy = (1/6.0)*(kvM[0][1][i] + 2*kvM[1][1][i] + 2*kvM[2][1][i] + kvM[3][1][i]);
    vM[1][i] = vM[1][i] + kmeanvy*dt;
    FLOAT kmeanvz = (1/6.0)*(kvM[0][2][i] + 2*kvM[1][2][i] + 2*kvM[2][2][i] + kvM[3][2][i]);
    vM[2][i] = vM[2][i] + kmeanvz*dt;
